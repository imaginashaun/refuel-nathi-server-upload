...
class DeliveryServiceController extends Controller
{
    ...
    public function AccountOrders($id){
        $orders=  DB::select("SELECT O.id,order_status_id,user_id from orders O INNER JOIN order_statuses OS ON OS.id=O.order_status_id  where user_id=? and active=1 and OS.can_message=1  and driver_id is not null",[$id]);
        if (count($orders)) {
