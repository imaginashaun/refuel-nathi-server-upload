...
class MarketAPIController extends Controller
{
    ...
    public function ClosestMarket(Request  $request)
    {
        $lat=$request->lat;
        $long=$request->long;
        $market =DB::select("select MIN(a.distance_in_km) ,distance_in_km,name,id FROM (SELECT id,name,delivery_range,
   111.111 *
  ...
  FROM markets  )a  WHERE a.delivery_range>distance_in_km  and a.distance_in_km is not null ",[$lat,$long,$lat]);
        if (count($market)) {

            if($market[0]->distance_in_km==null){
